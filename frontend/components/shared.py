...
def highlight_matching_segments(text, reference, confidence):
    ...
    Highlights matching words/phrases inside the text block relative to the reference.
    """
    if confidence < 0.4:
        return text
    
    # Normalize and tokenize reference into punctuation-stripped lowercase words
    ref_words = set()
    if reference:
        for r_word in reference.split():
            r_clean = r_word.strip(".,;:!?\"'()[]{}<>*-+=_/\\")
            if r_clean:
                ref_words.add(r_clean.lower())
    
    # Determine color, border color, and opacity based on confidence level
    if confidence >= 0.7:
        color_rgb = "244, 63, 94"  # Rose/Red
        border_color = "#f43f5e"
        alpha = min(0.9, 0.6 + (confidence - 0.7) * 0.5)
    else:
        color_rgb = "234, 179, 8"   # Amber/Yellow
        border_color = "#eab308"
    ...
    for token in tokens:
        ...
        if token.isspace():
            ...
            # Separate leading/trailing punctuation from the clean alphanumeric word
            leading_match = re.match(r'^[^a-zA-Z0-9]+', token)
            leading = leading_match.group(0) if leading_match else ""
            
            trailing_match = re.search(r'[^a-zA-Z0-9]+$', token)
            ...
            clean = token[len(leading):len(token)-len(trailing)]
            
            if clean.lower() in ref_words:
                highlighted_word = f'<span style="background-color: rgba({color_rgb}, {alpha:.2f}); font-weight: 600; padding: 1px 3px; border-radius: 3px;">{clean}</span>'
                result.append(f"{leading}{highlighted_word}{trailing}")
            else:
                result.append(token)
                
    highlighted_text = "".join(result)
    
    # Wrap in a clean block container with a left border highlight and very subtle background tint
    highlighted_html = f'<div style="background-color: rgba({color_rgb}, 0.04); border-left: 4px solid {border_color}; padding: 6px 12px; border-radius: 0 6px 6px 0; margin-bottom: 8px; line-height: 1.6; display: block;">{highlighted_text}</div>'
